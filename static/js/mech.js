...
function showresult(evt) {
  ...
  if (rollno.value == "" || rollno.value.length < 6) {
    rollno.focus();
  } else {
    console.log("create");
  ...
  if (value.length > 6) {
    for (i = 1; i < obj.length; i++) {
      ...
      if (obj[i][1] == value) {
        ...
        for (j = 1; j < col; j++) {
          if (
            tr1.childElementCount == 0 ||
            tr1.childElementCount < tr.childElementCount
          ) {
...
function Open_pdf(url) {
  ...
  document.getElementById("pdf-viewer").children[0].src =
    "/static/web/viewer.html?file=" + url;
  deleteChild();
  showpage();
  function showpage() {
